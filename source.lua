...
function NotificationSystem:RepositionNotifications()
    ...
    for i, noti in pairs(NotificationSystem.ActiveNotifications)do
        if noti.Gui and noti.Gui:FindFirstChild('Holder') then
            local holder = noti.Gui.Holder

            TweenService:Create(holder, TweenInfo.new(0.3, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
                Position = UDim2.new(1, -230, 1, -130 - currentOffset),
            }):Play()

            noti.Offset = currentOffset
            currentOffset = currentOffset + (noti.Height or 130) + 10
        end
    end
end
function NotificationSystem:Send(title, description, duration)
    ...
    local minHeight = 80
    local maxHeight = 150
    local descLength = #(description or '')
    local dynamicHeight = math.clamp(minHeight + math.floor(descLength / 25) * 10, minHeight, maxHeight)
    local holder = Instance.new('Frame', notiGui)

    holder.BorderSizePixel = 0
    holder.BackgroundColor3 = Color3.fromRGB(0, 0, 0)
    holder.Size = UDim2.new(0, 220, 0, dynamicHeight)
    ...
    for _, noti in pairs(NotificationSystem.ActiveNotifications)do
        offset = offset + (noti.Height or 130) + 10
    end
...
function Library:CreateWindow(title)
    ...
    function window:CreateTab(name)
        ...
        function tab:AddButton(opts)
            ...
            desc.Size = UDim2.new(0, 185, 0, 50)
            desc.Text = opts.Description or ''
            desc.Position = UDim2.new(0.3, 0, 0.036, 0)
